...
impl RedisTimeSeriesService {
    ...
    pub async fn add_point(&self, key: &str, point: &TimeSeriesPoint) -> RedisResult<bool> {
        ...
        match cmd.query_async::<()>(&mut conn).await {
            ...
            Err(_) => {
                ...
                debug!(
                    "Added point to fallback time series {}: {}",
                    key, point.value
                );
                Ok(true)
            }
    ...
    pub async fn query_range(
        ...
        let mut cmd = redis::Cmd::new();
        cmd.arg("TS.RANGE").arg(key).arg(range.start).arg(range.end);
        ...
        match cmd.query_async::<Vec<(i64, String)>>(&mut conn).await {
            Ok(results) => {
            ...
            Err(_) => {
                ...
                debug!(
                    "Queried {} points from fallback time series {}",
                    points.len(),
                    key
                );
                Ok(points)
    ...
    pub async fn get_latest(&self, key: &str) -> RedisResult<Option<TimeSeriesPoint>> {
        ...
        let mut cmd = redis::Cmd::new();
        cmd.arg("TS.GET").arg(key);

        match cmd.query_async::<Option<(i64, String)>>(&mut conn).await {
            Ok(Some((timestamp, value_str))) => {
            ...
            Err(_) => {
                // Fallback to sorted set
                let results: Vec<(String, f64)> = conn.zrevrange_withscores(key, 0, 0).await?;

                if let Some((timestamp, value)) = results.first() {
                    let ts = timestamp.parse::<i64>().unwrap_or(0);
    ...
    pub async fn list_time_series(&self, pattern: &str) -> RedisResult<Vec<String>> {
        ...
        debug!(
            "Found {} time series matching pattern: {}",
            keys.len(),
            pattern
        );
        Ok(keys)
    ...
    pub async fn compact(
        ...
        let points = self
            .query_range(source_key, &query_range, Some((aggregation, bucket_ms)))
            .await?;

        if !self
            .create_time_series(target_key, None, None)
            .await
            .unwrap_or(false)
        {
        ...
        info!(
            "Compacted {} points from {} to {}",
            added, source_key, target_key
        );
        Ok(added)
    }
}
...
impl GridTokenXTimeSeries {
    ...
    pub async fn record_energy_generation(
        &self,
        meter_id: &str,
        generation_kwh: f64,
    ) -> RedisResult<bool> {
    ...
    pub async fn get_trading_volume_stats(
        &self,
        range: &TimeRange,
