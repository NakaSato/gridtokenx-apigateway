...
impl RedisJSONService {
    ...
    pub async fn json_get(&self, key: &str, path: &str) -> RedisResult<Option<Value>> {
        ...
        match conn.query::<Option<String>>(&cmd) {
            Ok(Some(json_str)) => {
    ...
    pub async fn json_merge(&self, key: &str, path: &str, value: &Value) -> RedisResult<bool> {
        ...
        match conn.query::<()>(&cmd) {
            ...
            Err(_) => {
                ...
                if let Ok(mut existing_value) = self.json_get(key, path).await {
    ...
    pub async fn json_arr_len(&self, key: &str, path: &str) -> RedisResult<Option<u32>> {
        ...
        match conn.query::<Option<u32>>(&cmd) {
            Ok(result) => Ok(result),
            Err(_) => {
    ...
    pub async fn json_arr_append(
        ...
        match conn.query::<u32>(&cmd) {
            ...
            Err(_) => {
                ...
                if let Ok(Some(mut existing_value)) = self.json_get(key, path).await {
    ...
    pub async fn json_obj_keys(&self, key: &str, path: &str) -> RedisResult<Option<Vec<String>>> {
        ...
        match conn.query::<Option<Vec<String>>>(&cmd) {
            Ok(result) => Ok(result),
            Err(_) => {
    ...
    pub async fn json_num_incrby(
        ...
        match conn.query::<Option<f64>>(&cmd) {
            Ok(result) => {
