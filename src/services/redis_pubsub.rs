...
use redis::{AsyncCommands, Client, PubSub, RedisResult};
    ...
    pub fn new<T: Serialize>(
        ...
        Ok(Self {
            ...
            data: serde_json::to_value(data)
                .map_err(|e| redis::RedisError::from((redis::ErrorKind::TypeError, e.to_string(), e.to_string())))?,
...
impl RedisPubSubService {
    ...
    pub async fn initialize(&self, channels: &[&str]) -> RedisResult<()> {
        ...
        tokio::spawn(async move {
            while let Some(msg) = pubsub.on_message().next().await {
                ...
                // Parse the message
                if let Ok(pubsub_msg) = serde_json::from_str::<PubSubMessage>(&payload) {
                    // Broadcast to local subscribers
                    if let Some(subscribers_map) = subscribers_clone.read().await.get(channel) {
                        for rx in subscribers_map.iter() {
    ...
    pub async fn publish_market_event(&self, event: MarketEvent) -> RedisResult<()> {
        match &event {
            ...
            MarketEvent::TradeExecuted { .. } => {
                self.publish(channels::MARKET_EVENTS, "trade_executed", event).await
            }
        }
    }
    ...
    pub async fn publish_system_alert(&self, alert: SystemAlert) -> RedisResult<()> {
        self.publish(channels::SYSTEM_ALERTS, "system_alert", alert).await
    }

    /// Publish user notification
    pub async fn publish_user_notification(&self, user_id: &str, notification: UserNotification) -> RedisResult<()> {
        self.publish(
    ...
    pub async fn subscribe(&self, channel: &str) -> RedisResult<broadcast::Receiver<PubSubMessage>> {
        let mut subscribers = self.subscribers.write().await;
        
        if let Some(subscriber_list) = subscribers.get_mut(channel) {
            let (tx, rx) = broadcast::channel(1000);
            subscriber_list.push(rx);
    ...
    pub async fn get_channel_stats(&self, channel: &str) -> RedisResult<HashMap<String, i64>> {
        ...
        let subscriber_count = self.subscribers.read().await
            .get(channel)
...
impl WebSocketPubSubBridge {
    ...
    pub async fn add_connection(&self, connection_id: String, channels: Vec<String>) -> RedisResult<()> {
        let (tx, _) = broadcast::channel(1000);
                ...
                while let Ok(message) = rx.recv().await {
                    if let Some(sender) = connections_clone.read().await.get(&connection_id_clone) {
                        if let Err(e) = sender.send(message.clone()) {
                            warn!("Failed to send message to WebSocket {}: {}", connection_id_clone, e);
                            break;
                        }
                    } else {
                        break;
        ...
        info!("Added WebSocket connection {} to channels: {:?}", connection_id, channels);
        Ok(())
    }
