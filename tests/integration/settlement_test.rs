...
// - SettlementService::new() now requires encryption_secret parameter
// - Import paths have changed: market -> market_clearing, settlement_service -> settlement  
// - SettlementConfig and SettlementStatus are in settlement::types
...
async fn setup_settlement_test() -> Result<(PgPool, Arc<BlockchainService>, SettlementService, Uuid)>
{
    ...
    // Initialize settlement service with test encryption secret
...
async fn test_create_settlement_from_trade() -> Result<()> {
    let (db_pool, _blockchain_service, settlement_service, epoch_id): (PgPool, Arc<BlockchainService>, SettlementService, Uuid) =
        setup_settlement_test().await?;
...
async fn test_settlement_fee_calculation() -> Result<()> {
    let (db_pool, blockchain_service, _, epoch_id): (PgPool, Arc<BlockchainService>, SettlementService, Uuid) = setup_settlement_test().await?;
    ...
    for (fee_rate, description) in test_cases {
        ...
        let settlement_service =
            SettlementService::with_config(db_pool.clone(), (*blockchain_service).clone(), config, encryption_secret);
...
async fn test_complete_settlement_workflow() -> Result<()> {
    let (db_pool, blockchain_service, settlement_service, epoch_id): (PgPool, Arc<BlockchainService>, SettlementService, Uuid) =
        setup_settlement_test().await?;
