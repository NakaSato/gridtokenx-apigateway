...
CREATE INDEX idx_trading_orders_active ON trading_orders(status, price_per_kwh)
...
CREATE INDEX idx_trading_orders_type_status_price ON trading_orders(order_type, status, price_per_kwh)
...
CREATE INDEX idx_order_matches_epoch_status ON order_matches(epoch_id, status);
...
CREATE INDEX idx_settlements_buyer_created ON settlements(buyer_id, created_at DESC);
CREATE INDEX idx_settlements_seller_created ON settlements(seller_id, created_at DESC);

-- Index for pending settlements
CREATE INDEX idx_settlements_pending ON settlements(status, created_at)
...
CREATE INDEX idx_certificates_wallet_status ON energy_certificates(wallet_address, status)
WHERE status = 'active';

-- Index for expiring certificates
CREATE INDEX idx_certificates_expiry ON energy_certificates(expiry_date)
...
CREATE INDEX idx_market_epochs_status_time ON market_epochs(status, start_time DESC);
...
CREATE INDEX idx_certificates_metadata ON energy_certificates USING GIN(metadata jsonb_path_ops)
...
CREATE INDEX idx_trading_orders_user_covering ON trading_orders(user_id, status)
