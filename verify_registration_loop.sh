...
step_1_add_meter() {
    ...
    # Create meter via API
    RES=$(curl -s -X POST "${SIMULATOR_URL}/api/meters/add" \
        -H "Content-Type: application/json" \
        -d "{
            \"meter_type\": \"Solar_Prosumer\",
            \"location\": \"Loop Test Zone\",
            \"solar_capacity\": 5.0,
            \"battery_capacity\": 10.0
        }")
    
    METER_ID=$(get_json_field "$RES" "meter.meter_id")
    print_success "Meter ID: $METER_ID"
    
    if [ -n "$METER_ID" ]; then
        print_success "Created Meter ID: $METER_ID"
    else
        print_error "Failed to create meter. Response: $RES"
        # Fallback to existing
        METER_ID=$(curl -s "${SIMULATOR_URL}/api/meters/" | jq -r '.meters[0].meter_id // empty')
        if [ -n "$METER_ID" ]; then
    ...
    if command -v solana-keygen &> /dev/null; then
        ...
        solana-keygen new --no-bip39-passphrase --outfile /tmp/temp-meter-key.json > /dev/null 2>&1
        METER_PUBLIC_KEY=$(solana-keygen pubkey /tmp/temp-meter-key.json)
        rm /tmp/temp-meter-key.json
        print_info "Generated real Solana Key: $METER_PUBLIC_KEY"
    else
        # Fallback to a valid hardcoded pubkey (System Program ID) or pre-calculated one as valid BS58
